...
const myxss = new FilterXSS({ whiteList: { ...whiteList, span: ['class'] } });
  ...
  return (
    ...
    >
      ...
      <h2 className="text-3xl md:text-5xl lg:text-6xl font-black text-primary uppercase tracking-tighter mb-4 leading-tight">
        {title ? (
            <span dangerouslySetInnerHTML={{ __html: myxss.process(title).replace('transforma', '<span class="text-transparent bg-clip-text bg-gradient-to-r from-accent to-secondary">transforma</span>') }} />
        ) : (
