    ...
    >
      ...
      <h2 className="text-3xl md:text-5xl lg:text-6xl font-black text-primary uppercase tracking-tighter mb-4 leading-tight">
        {title ? (
            <span>
              {title.split(/(transforma)/i).map((part, index) =>
                part.toLowerCase() === 'transforma' ? (
                  <span key={index} className="text-transparent bg-clip-text bg-gradient-to-r from-accent to-secondary">{part}</span>
                ) : (
            ...
            <span>Tu apoyo <span className="text-transparent bg-clip-text bg-gradient-to-r from-accent to-secondary">transforma</span> vidas</span>
        )}
      </h2>
