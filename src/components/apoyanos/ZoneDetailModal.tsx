...
const typeIcons: Record<DonationType, React.ComponentType<{ className?: string }>> = {
        ...
        <>
          ...
          >
            ...
            <div className="p-6 bg-gray-50 border-b">
              <div className="flex justify-between items-center mb-3">
                ...
                <span className="text-sm text-gray-600">
                  {formatCurrency(zone.totalRaised)} / {formatCurrency(zone.totalNeeded)}
                </span>
              </div>
                    ...
                    >
                      ...
                      <div className="mt-3">
                        <div className="flex justify-between text-sm mb-1">
                          <span className="text-gray-600">
                            {need.quantityDonated} / {need.quantityNeeded} {need.unit}
                          </span>
                          <span className="font-bold text-primary">
                            {formatCurrency(need.unitPrice)}/{need.unit}
                          </span>
                        </div>
                        <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                          <div 
                            className={`h-full rounded-full ${isComplete ? 'bg-green-500' : 'bg-accent'}`} 
                            style={{ width: `${Math.min(donatedPercentage, 100)}%` }} 
                          />
                        </div>
                      </div>
                      ...
                      {!isComplete && (
                        <button
                          ...
                          className="mt-3 w-full py-2 bg-primary text-white rounded-lg font-medium text-sm hover:bg-secondary transition-colors"
                        >
                          Donar {need.unit} de {need.name}
                        </button>
                      )}
                    </motion.div>
                  );
            ...
            <div className="p-6 border-t bg-gray-50">
              <button
                onClick={openDonationWidget}
                className="w-full py-4 bg-accent text-primary rounded-xl font-bold text-lg hover:bg-yellow-400 transition-colors"
              >
                Donar a esta zona
              </button>
            </div>
